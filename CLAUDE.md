...
All three read their version from the root `package.json` — bump that (and `desktop/package.json` / `desktop/src-tauri/Cargo.toml` for consistency, though only the root one is functionally load-bearing) before tagging. The desktop build is the slow part: a fresh Rust dependency compile takes ~15 minutes on a cold cache; `Swatinem/rust-cache` should make subsequent releases faster. **Code signing goes through SignPath Foundation's free open-source program, and is gated.** `release.yml`'s signing steps run only when the repository variable `SIGNPATH_ORGANIZATION_ID` is set (plus `SIGNPATH_PROJECT_SLUG`, `SIGNPATH_SIGNING_POLICY_SLUG`, and secret `SIGNPATH_API_TOKEN`); without it a release publishes unsigned, exactly as before, and the release notes say so. Things to know before touching it:

- **Signing must happen before `Checksum`.** Authenticode changes each file's bytes, so `SHA256SUMS.txt` (and any winget manifest hash) has to be computed from the signed files.
- **The three executables are staged under fixed, unversioned names** for signing and renamed back afterwards, so `.signpath/artifact-configuration.xml` can list exact paths. That XML is a reference copy — SignPath reads the one in its web UI, so change both together.
- **Every signing request needs a person to approve it in SignPath.** The job sits waiting (timeout set to 5 hours) until someone does; a release that seems stuck on "Sign with SignPath" is usually waiting on approval.
- **Inno Setup pads its version strings with trailing spaces**, which may trip SignPath's exact `product-name`/`product-version` match on `ai-quota-meter-setup.exe` only. If that is the first signing failure, relax that one entry, not the others.
- **Only the three downloaded executables are signed.** The binaries *inside* the installers (the Tauri shell, the sidecar, the uninstallers) are not. SmartScreen only checks downloaded files, so this clears the download warning, but Windows 11 Smart App Control may still object to the unsigned inner files.
- **SignPath's terms require a public "Code signing policy"** (README section + `CODE_SIGNING.md`): attribution line, team roles, privacy statement. Its privacy wording claims the app contacts nothing unless the user asks — keep it true. Background polling, update checks, or telemetry would each make it false.

CI publishes a GitHub Release on every matching tag push, and there's no draft/staging step — test signing changes on a real version, not a `-test` tag.
